...
Look at predictions estimated by-variate Gaussian:
```{r plot_ga6, fig.width = 7, fig.height = 5}
preds <- predict(fit2, type = "response")
head(preds)
```

```{r plot_ga0, dev='png', fig.width = 6, fig.height = 6, fig.show='hide'}
library(mvnfast)
ii <- 1
...
![Bi-variate demand distribution (centered at zero)](`r knitr::fig_chunk('plot_ga0', 'png')`)
